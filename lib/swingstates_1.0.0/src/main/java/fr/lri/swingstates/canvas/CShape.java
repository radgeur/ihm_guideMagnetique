 ...
 * another shape in the canvas. In this case the shape's transform is a 
 * composition of the transform of its parent and its own transform.
...
, GraphicalShape
{
	...
	protected Shape strokedShape = null;

	// transformation
	protected double rx = 0.5, ry = 0.5; // the reference point relative to bounding box. Default = center of bounding box
	...
	public void paint(Graphics g) {
		...
		if (clip != null && (canvas != null && clip != canvas.clip)) {
			if (clip == DEFAULT_CLIP) {
				g2d.clipRect(0, 0, canvas.getWidth(), canvas.getHeight());
			} else {
				g2d.transform(clip.getAbsTransform());
				g2d.clip(clip.getShape());
	 ...
	 * The reference point is relative to the
	...
	public Point2D shapeToCanvas(Point2D p) {
		...
		Rectangle2D bounds = shape.getBounds2D();
		Point2D pRelativeToShape = new Point2D.Double(p.getX()+bounds.getMinX(), p.getY()+bounds.getMinY());
		Point2D q = new Point2D.Double();
		AffineTransform xf = getAbsTransform();
		xf.transform(pRelativeToShape, q);
		return q;
	}
	 ...
	 * For example, if p is (120, 300) and the shape s is located at (100, 120), s.canvasToShape(p) will return (20, 180).
	 * 
	 * @param p
	 *            The point to be converted
	 * @return the converted point
	...
	public Point2D canvasToShape(Point2D p) {
		...
		//		ptDst.setLocation( (ptDst.getX() - shape.getBounds2D().getMinX())/shape.getBounds2D().getWidth(), (ptDst.getY() - shape.getBounds2D().getMinY())/shape.getBounds2D().getHeight());
		return ptDst;
	}
	 ...
	 * For example, if p is (120, 300) and the shape s is located at (100, 120) and has a width of 80 and a height of 360, s.canvasToShape(p) will return (0.25, 0.5) (i.e. ((120-100)/80, (300-120)/360)).
	 * 
	 * @param p
	 *            The point to be converted
	 * @return the converted point
	 */
	public Point2D canvasToShapeRef(Point2D p) {
		Point2D ptDst = new Point2D.Double();
		try {
			absTransform.inverseTransform(p, ptDst);
		} catch (NoninvertibleTransformException e) {
	...
	/**
	 * @return the X coordinate of the center of the bounding box of
	 *         this shape in double precision.
	 */
	...
	/**
	 * @return the Y coordinate of the center of the bounding box of
	 *         this shape in double precision.
	 */
	...
	public CShape contains(Point2D p) {
		...
		try {
			absTransform.inverseTransform(p, ptDst);
			return shape.contains(ptDst) ? this : null;
		} catch (NoninvertibleTransformException e) {
			return null;
		}
	}
	 ...
	 * This method takes into account the clipping shape, i.e. the point is contained
	 * in the shape AND in its clipping shape.
	 * 
	 * @param p
	 *            The point
	 * @see java.awt.Shape#contains(Point2D)
	...
	public CShape pick(Point2D p, int tolerance) {
		...
		try {
			...
			if ((filled && shape.contains(ptDst)) 
					|| (outlined && strokedShape.intersects(pickingRectangle)))
	...
	public CShape isOnOutline(Point2D p) {
		...
		try {
			...
			if (strokedShape.intersects(pickingRectangle))
				return this;
	 ...
	 * that canvas, on top of all other objects; The copy has no children, i.e. it
