...
import fr.lri.swingstates.gestures.clement.ClementClassifier;
...
//	private JSpinner probability;
	...
	private JSpinner distanceClement;
...
//	private JTextArea recognitionInfoTextArea = new JTextArea();
	...
	protected Training() {
		...
		panelRecognitionInfo = new JPanel(new GridLayout(1, 3));
		jspRecognitionInfo = new JScrollPane(panelRecognitionInfo);
		...
		clementList.setBorder(BorderFactory.createEtchedBorder());
		...
		clementList.setForeground(Color.DARK_GRAY);
		clementList.setFont(currentFont);
		clementList.setEditable(false);
		panelRecognitionInfo.add(rubineList);
		panelRecognitionInfo.add(dollar1List);
		panelRecognitionInfo.add(clementList);
//		recognitionInfoFrame.getContentPane().setLayout(new GridLayout(1, 3));
//		recognitionInfoFrame.getContentPane().add(rubineList);
//		recognitionInfoFrame.getContentPane().add(dollar1List);
//		recognitionInfoFrame.getContentPane().add(clementList);
	...
	protected void openFile(File f) {
		...
		ClementClassifier clementClassifier = ClementClassifier.newClassifier(f);
//		probability.setValue(rubineClassifier.getMinimumProbability());
		...
		classifiers.add(clementClassifier);
...
//		});
		...
		SpinnerNumberModel modelDistanceClement = new SpinnerNumberModel(new Integer(Integer.MAX_VALUE), new Integer(0), new Integer(Integer.MAX_VALUE), new Integer(1));
		distanceClement = new JSpinner(modelDistanceClement);
		distanceClement.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				// TODO
//				getClementClassifier().setThreshold((Double) (((SpinnerNumberModel) distanceClement.getModel()).getNumber()));
			}
		});
	}
	...
	private ClementClassifier getClementClassifier() {
		return (ClementClassifier) classifiers.get(INDEX_CLEMENT);
	}
...
//		topPanel.add(probability);
		...
		final JPanel clementPanel = new JPanel(new BorderLayout());
		clementPanel.setBorder(BorderFactory.createEtchedBorder());
		topPanel.add(rubinePanel);
		topPanel.add(dollar1Panel);
		topPanel.add(clementPanel);
		...
		clementEnable = new JCheckBox("Clement");
		clementEnable.setFont(currentFont);
		clementPanel.add(clementEnable, BorderLayout.NORTH);
		
		final JLabel distanceClementLabel = new JLabel("Distance max: ");
		distanceClementLabel.setFont(currentFont);
		clementPanel.add(distanceDollar1Label, BorderLayout.WEST);

		distanceClement.setPreferredSize(new Dimension(100, distanceClement.getPreferredSize().height));
		clementPanel.add(distanceClement, BorderLayout.CENTER);

		clementEnable.setSelected(true);
		clementEnable.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if(e.getStateChange() == ItemEvent.DESELECTED) {
					distanceClementLabel.setForeground(Color.LIGHT_GRAY);
					distanceClementLabel.setEnabled(false);
					distanceClement.setEnabled(false);
				} else {
					distanceClementLabel.setForeground(Color.BLACK);
					distanceClementLabel.setEnabled(true);
					distanceClement.setEnabled(true);
				}
			}
		});
...
//			textResult += "\n" + "\tProbability of non ambiguity: " + getRubineClassifier().getCurrentProbability();
			...
			if(!panelRecognitionInfo.isAncestorOf(rubineList)) {
				panelRecognitionInfo.removeAll();
				panelRecognitionInfo.add(rubineList);
			}
		} else {
		...
		if(dollar1Enable.isSelected()) {
			...
			try {
				textResult += "    " + getDollar1Classifier().classify(gest);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			if(!panelRecognitionInfo.isAncestorOf(dollar1List)) {
				panelRecognitionInfo.remove(clementList);
				panelRecognitionInfo.add(dollar1List);
			}
		} else {
		...
		if(clementEnable.isSelected()) {
			String textResult = "Clement classifier has recognized: \n";
			try {
				textResult += "    " + getClementClassifier().classify(gest);
			} catch (Exception e) {
				e.printStackTrace();
			}
			textResult += "\n" + "Distance: " + getClementClassifier().getCurrentDistance();
			String sortedClasses = "\n\nClasses order (closest first): \n";
			Vector<Score> sorted = getClementClassifier().sortedClasses(gest);
			for (Iterator<Score> iterator = sorted.iterator(); iterator.hasNext();) {
				Score next = iterator.next();
				sortedClasses += "\n    " + next.getName()+" ("+next.getScore()+")";
			}
			...
			clementList.setText(textResult);
			if(!panelRecognitionInfo.isAncestorOf(clementList)) {
				panelRecognitionInfo.add(clementList);
			}
		} else {
			panelRecognitionInfo.remove(clementList);
		...
		jspRecognitionInfo.setPreferredSize(new Dimension((int)Math.min(1000, panelRecognitionInfo.getPreferredSize().getWidth()), 300));
