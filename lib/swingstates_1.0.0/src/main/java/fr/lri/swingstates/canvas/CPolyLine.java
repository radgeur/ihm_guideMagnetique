...
public class CPolyLine extends CShape {
	...
	private void init(double x, double y) {
		...
		((GeneralPath) getShape()).moveTo((float) x, (float) y);
		setShape(getShape());
		repaint();
	}
	...
	private void initIfEmpty() {
		if (((GeneralPath) getShape()).getCurrentPoint() == null) {
			init(0, 0);
		}
	}
	...
	public CPolyLine lineTo(double x, double y) {
		initIfEmpty();
		((GeneralPath) getShape()).lineTo((float) x, (float) y);
		setShape(getShape());
	...
	public CPolyLine arcTo(double start, double extent, double rx, double ry) {
		...
		((GeneralPath) getShape()).append(arc.getPathIterator(null), true);
		setShape(getShape());
	...
	public Point2D getCurrentPoint() {
		return ((GeneralPath) getShape()).getCurrentPoint();
	}
	...
	public CPolyLine quadTo(double ctrlx1, double ctrly1, double xEnd, double yEnd) {
		initIfEmpty();
		((GeneralPath) getShape()).quadTo((float) ctrlx1, (float) ctrly1, (float) xEnd, (float) yEnd);
		setShape(getShape());
	...
	public CPolyLine curveTo(double ctrlx1, double ctrly1, double ctrlx2, double ctrly2, double xEnd, double yEnd) {
		initIfEmpty();
		((GeneralPath) getShape()).curveTo((float) ctrlx1, (float) ctrly1, (float) ctrlx2, (float) ctrly2, (float) xEnd, (float) yEnd);
		setShape(getShape());
	...
	public CPolyLine close() {
		((GeneralPath) getShape()).closePath();
		setShape(getShape());
		repaint();
		return this;
	}
	...
	public CPolyLine removeLastSegment() {
		PathIterator iter = ((GeneralPath) getShape()).getPathIterator(null);
