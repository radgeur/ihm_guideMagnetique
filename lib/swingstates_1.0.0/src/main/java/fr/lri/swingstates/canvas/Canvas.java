...
import fr.lri.swingstates.canvas.transitions.EventOnShape;
...
import fr.lri.swingstates.canvas.transitions.WheelOnShape;
import fr.lri.swingstates.canvas.transitions.WheelOnTag;
...
MouseMotionListener, MouseWheelListener, KeyListener, CElement {
	 ...
	 */
	protected CWidget widgetFocused = null;

	/**
	 * The current picked shape for each active picker.
	 */
	protected Vector<CShape> pickedShapes = new Vector<CShape>();
	 ...
	 */
	protected void processEvent(InputEvent e) {
		CStateMachine machine;
		boolean isConsumed = false;
		synchronized(stateMachines) {
			Iterator<CStateMachine> i = stateMachines.iterator(); // Must be in synchronized block
	...
	protected void pickAndProcess(MouseEvent event) {
		Picker eventPicker;
	...
	private void updatePicker(Picker movedPicker, int modifiersEx) {
		...
		if (previousPicked != newPicked) {
			...
			if (previousPicked != null) {
				dispatchEvent(new PickerCMouseEvent(this, previousPicked,
			...
			if (newPicked != null) {
				dispatchEvent(new PickerCMouseEvent(this, newPicked, movedPicker,
	 ...
	 */
	protected void processModifiedEvent(Picker eventPicker, MouseEvent initialEvent) {
		int index = pickers.indexOf(eventPicker);
		CShape picked = pickedShapes.get(index);
		if (picked != null) {
			if(initialEvent.getID() == MouseEvent.MOUSE_WHEEL) {
				MouseWheelEvent eventToDispatch = new PickerCMouseWheelEvent(
						(Component) initialEvent.getSource(), picked, eventPicker,
						initialEvent.getID(), initialEvent.getWhen(), 
						initialEvent.getModifiersEx(),
						(int) initialEvent.getPoint().getX(), (int) initialEvent.getPoint().getY(),
...
//				System.out.println("\nWHEEL\n*** "+eventToDispatch.getModifiers()+" / "+initialEvent.getModifiers()+" ***");
//				System.out.println("*** "+eventToDispatch.getModifiersEx()+" / "+initialEvent.getModifiersEx()+" ***");
//				System.out.println("*** "+Utils.button(eventToDispatch)+" / "+Utils.button(initialEvent)+" ***");
				processEvent(eventToDispatch);
			} else {
				MouseEvent eventToDispatch = new PickerCMouseEvent((Component) initialEvent.getSource(),
						picked, eventPicker, initialEvent.getID(),
						initialEvent.getWhen(),
						initialEvent.getModifiersEx(),
						(int) initialEvent.getPoint().getX(), (int) initialEvent.getPoint().getY(), 
						initialEvent.getClickCount(), initialEvent.isPopupTrigger(), initialEvent.getButton());
//				System.out.println("\nNOT WHEEL\n*** "+eventToDispatch.getModifiers()+" / "+initialEvent.getModifiers()+" ***");
//				System.out.println("*** "+eventToDispatch.getModifiersEx()+" / "+initialEvent.getModifiersEx()+" ***");
//				System.out.println("*** "+Utils.button(eventToDispatch)+" / "+Utils.button(initialEvent)+" ***");
				processEvent(eventToDispatch);
			...
			if (picked instanceof CWidget) {
				...
				if (widgetFocused.isBasicListener()) {
					widgetFocused.sendEvent(eventPicker.getLocation(), initialEvent);
				}
			}
		} else
	...
	public CShape pick(Point2D p) {
		for (ListIterator<CShape> i = displayOrder.listIterator(displayOrder.size()); i.hasPrevious();) {
			CShape sms = i.previous();
			if (sms.isPickable())
	...
	public LinkedList<CShape> pickAll(Point2D p) {
		...
		for (ListIterator<CShape> i = displayOrder.listIterator(0); i.hasNext();) {
			CShape sms = i.next();
			if (sms.isPickable())
				if (sms.pick(p, 2) != null)
...
//		displayOrder.removeAll(taggedShapes);
		for(Iterator<CShape> it = displayOrder.iterator(); it.hasNext(); ) {
			if(taggedShapes.contains(it.next())) {
				it.remove();
			}
		}
		repaint();
	...
	public Canvas removeAllShapes() {
		// Copy displayOrder to avoid any co-modification that may occur during prepareToRemove
		List<CShape> displayOrderCopy = new LinkedList<CShape>();
		for (Iterator<CShape> i = displayOrder.iterator(); i.hasNext();)
			displayOrderCopy.add(i.next());
	 ...
	 * @return the newly created image.
	 */
	public CImage newImage(double x, double y, BufferedImage image) {
		CImage s = new CImage(image, new Point2D.Double(x, y));
		addShape(s);
		return s;
	}	
