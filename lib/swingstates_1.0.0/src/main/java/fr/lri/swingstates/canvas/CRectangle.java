...
public class CRectangle extends CRectangularShape {
	...
	public CRectangle roundCorners(double arcw, double arch) {
		if(getShape() instanceof RoundRectangle2D) {
			if(((RoundRectangle2D)getShape()).getArcWidth() == arcw 
					&& ((RoundRectangle2D)getShape()).getArcHeight() == arch)
				return this;
		}
		setShape(new RoundRectangle2D.Double(
				getShape().getBounds2D().getMinX(),
				getShape().getBounds2D().getMinY(),
				getShape().getBounds2D().getWidth(),
				getShape().getBounds2D().getHeight(),
				arcw, arch));
		repaint();
		return this;
	}
