...
public class Utils {
	...
	public static int modifiers(InputEvent e){
		...
		boolean isControlDown = ((InputEvent.CTRL_DOWN_MASK & e.getModifiersEx()) == InputEvent.CTRL_DOWN_MASK);
		boolean isShiftDown = ((InputEvent.SHIFT_DOWN_MASK & e.getModifiersEx()) == InputEvent.SHIFT_DOWN_MASK);
		boolean isAltDown = ((InputEvent.ALT_DOWN_MASK & e.getModifiersEx()) == InputEvent.ALT_DOWN_MASK);
		
		if(isControlDown) {
			if(isShiftDown) {
				if(isAltDown) return BasicInputStateMachine.ALT_CONTROL_SHIFT;
				return BasicInputStateMachine.CONTROL_SHIFT;
			}
			if(isAltDown) return BasicInputStateMachine.ALT_CONTROL;
			return BasicInputStateMachine.CONTROL;
		}
		if(isShiftDown) {
			if(isAltDown) return BasicInputStateMachine.ALT_SHIFT;
			return BasicInputStateMachine.SHIFT;
		}
		if(isAltDown) return BasicInputStateMachine.ALT;
	...
	public static int button(InputEvent e){
		int whichButton=e.getModifiersEx();
		if ((whichButton & InputEvent.BUTTON1_DOWN_MASK)==InputEvent.BUTTON1_DOWN_MASK)
			return BasicInputStateMachine.BUTTON1;
		else
			if ((whichButton & InputEvent.BUTTON2_DOWN_MASK)==InputEvent.BUTTON2_DOWN_MASK)
				return BasicInputStateMachine.BUTTON2;
			else 
				if ((whichButton & InputEvent.BUTTON3_DOWN_MASK)==InputEvent.BUTTON3_DOWN_MASK)
					return BasicInputStateMachine.BUTTON3;
				else {
					whichButton=e.getModifiers();
