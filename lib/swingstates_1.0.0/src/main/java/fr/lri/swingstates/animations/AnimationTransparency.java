		...
		if(Canvas.class.isAssignableFrom(ce.getClass())) {
			...
			if(CTag.class.isAssignableFrom(ce.getClass())) {
				...
				for(l.reset(); l.hasNext();) {
					...
					animatedShapes[cpt] = next;
					initialTransparencyOutline[cpt] = next.getTransparencyOutline() != null ? next.getTransparencyOutline().getAlpha() : 1;
					initialTransparencyFill[cpt] = next.getTransparencyFill() != null ? next.getTransparencyFill().getAlpha() : 1;
					cpt++;
				}
			} else {
				if(CShape.class.isAssignableFrom(ce.getClass())) {
					...
					initialTransparencyOutline[0] = ((CShape)ce).getTransparencyOutline() != null ? ((CShape)ce).getTransparencyOutline().getAlpha() : 1;
					initialTransparencyFill[0] = ((CShape)ce).getTransparencyFill() != null ? ((CShape)ce).getTransparencyFill().getAlpha() : 1;
				}
			}
		}
