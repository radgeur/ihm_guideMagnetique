...
public class CWidget extends CShape {
	...
	private Component deepestComponentPicked = null;
	...
	protected void init(JComponent w, double x, double y) {
		...
		setShape(new Rectangle2D.Double(x, y, size.getWidth(), size.getHeight()));
	 ...
	 * an input event occurs on it.
	 * @return this <code>CWidget</code>.
	 */
	public CElement widgetChanged() {
		biContext.setColor(widget.getBackground());
		biContext.fillRect(0, 0, widget.getWidth(), widget.getHeight());
	...
	CWidget sendEvent(Point2D point2D, MouseEvent evt) {
		...
		try {
			...
			deepestComponentPicked = SwingUtilities.getDeepestComponentAt(widget, (int)pt.getX(), (int)pt.getY());

			if(deepestComponentPicked == null) {
				...
				Point pt2 = SwingUtilities.convertPoint(widget, (int)pt.getX(), (int)pt.getY(), deepestComponentPicked);
				MouseEvent newEvent = new MouseEvent(deepestComponentPicked, evt.getID(), evt.getWhen(), evt.getModifiers(), (int)pt2.getX(), (int)pt2.getY(), evt.getClickCount(), evt.isPopupTrigger());
				deepestComponentPicked.dispatchEvent(newEvent);
			}
			widgetChanged();
		} catch (Exception e2) {
			deepestComponentPicked = null;
			// e2.printStackTrace();
	...
	CWidget sendEvent(KeyEvent evt) {
		...
		if(deepestComponentPicked != null) 
			deepestComponentPicked.requestFocus();
		else
			widget.requestFocus();

		if(deepestComponentPicked != null) {
			evt.setSource(deepestComponentPicked);
			deepestComponentPicked.dispatchEvent(evt);
		} else {
			evt.setSource(widget);
	...
	public CShape copyTo (CShape sms) {
		...
		if (sms instanceof CWidget) {
			...
			smi.init(widget, getShape().getBounds2D().getMinX(), getShape().getBounds2D().getMinY());
		}
		return this;
	}
