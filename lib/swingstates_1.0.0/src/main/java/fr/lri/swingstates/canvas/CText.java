...
public class CText extends CShape {
	...
	public CText setText(String text) {
		if(getShape() == null) {
			...
			setText(text, new Point2D.Double(getShape().getBounds().getX(), getShape().getBounds().getY()));
		}
		return this;
	}
	...
	private CText setText(String text, Point2D location) {
		...
		setShape(new Rectangle2D.Double(location.getX(), location.getY(), bounds.getWidth(), bounds.getHeight()));
